...
sudo update-alternatives --config x-session-manager
```
Chooe anything related to PIXEL(WIP: check the name for this)
##### LXDE
Same, make sure Xorg is installed.
```bash
sudo apt install lxde-core lxappearance
sudo update-alternatives --config x-session-manager
```
This one has to be LXDE, check that @im-coder-lg
# End
Well, hope you like this! This is 149 lines(raw mode on Gitpod) of Markdown code and the 150th one is blank. EOB(End of Blog). See ya!
